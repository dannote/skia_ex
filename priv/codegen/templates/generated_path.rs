fn build_path(path_term: Term) -> NifResult<skia_safe::Path> {
    ...
    for segment in segments.into_iter().rev() {
        ...
        } else if let Ok((op, cx, cy, x, y, weight)) =
            segment.decode::<(Atom, f64, f64, f64, f64, f64)>()
        {
            if op == atoms::conic_to() {
                builder.conic_to((cx as f32, cy as f32), (x as f32, y as f32), weight as f32);
            } else if op == atoms::r_conic_to() {
                builder.r_conic_to((cx as f32, cy as f32), (x as f32, y as f32), weight as f32);
            }
        } else if let Ok((op, x, y, width, height, start, arc_opts)) =
            segment.decode::<(Atom, f64, f64, f64, f64, f64, Term)>()
        {
            if op == atoms::arc_to() {
        ...
        } else if let Ok((op, rx, ry, x_axis_rotate, arc_opts)) =
            segment.decode::<(Atom, f64, f64, f64, Term)>()
        {
