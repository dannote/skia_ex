...
fn draw_rect_impl<'a>(
    ...
    if let Some(mut paint) = opt_fill_paint(raw_opts, atoms::fill())? {
        ...
        draw_rect_shape(surface, rect, radius, &paint);
    }
...
fn draw_circle_impl<'a>(
    ...
    if let Some(mut paint) = opt_fill_paint(raw_opts, atoms::fill())? {
        apply_blend_mode(&mut paint, raw_opts)?;
        surface.canvas().draw_circle(center, opts.radius, &paint);
    }
    if let Some(color) = opt_color(raw_opts, atoms::stroke())? {
        let paint = stroke_paint(color, opts.stroke_width.unwrap_or(1.0), raw_opts)?;
        surface.canvas().draw_circle(center, opts.radius, &paint);
...
fn draw_line_impl<'a>(
    ...
    surface
        .canvas()
        .draw_line(point_from_term(opts.from)?, point_from_term(opts.to)?, &paint);
    Ok(())
}
fn draw_rect_shape(
    surface: &mut skia_safe::Surface,
    ...
    if radius > 0.0 {
        surface.canvas().draw_rrect(RRect::new_rect_xy(rect, radius, radius), paint);
    } else {
