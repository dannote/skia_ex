defmodule Skia.Codegen.Native.Schema do
  @moduledoc """
  ...
  @spec index() :: Index.t()
  def index do
    Index.cached_package("skia-safe", manifest_path: @native_manifest)
  end
  ...
  def methods(target) when is_binary(target) do
    target
    |> normalize_target()
    |> then(&Index.methods(index(), &1))
  end

  @spec method!(String.t(), String.t()) :: method()
  def method!(target, name) when is_binary(target) and is_binary(name) do
    Index.method!(index(), normalize_target(target), name)
  rescue
    error in [RuntimeError] ->
      reraise RuntimeError,
              [message: "cannot find skia_safe::#{target}::#{name}: #{error.message}"],
              __STACKTRACE__
  end

  @spec descriptor!(NativeRef.t()) :: NativeDescriptor.t()
  def descriptor!(%NativeRef{} = ref) do
    NativeDescriptor.resolve!(index(), ref)
  end

  @spec descriptor!(String.t(), String.t()) :: NativeDescriptor.t()
  def descriptor!(target, name) when is_binary(target) and is_binary(name) do
    descriptor!(NativeRef.new(target, name, package: "skia-safe"))
  end

  @spec assert_method_shape!(String.t(), String.t(), keyword()) :: NativeDescriptor.t()
  def assert_method_shape!(target, name, opts) do
    target
    |> descriptor!(name)
    |> NativeDescriptor.assert_shape!(opts)
  end
  ...
  def safe_enum_type!(binding_enum) when is_binary(binding_enum) do
    Index.public_type_name!(index(), binding_enum)
  end
